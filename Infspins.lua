-- the script is open spurce the credit to owner 
-- this script is open sourced so yea if you do anything with it atleast give me credits tyy :333333334
...
local window = library:MakeWindow({
    Name = "ARBIX HUB INF SPINS ALSO GET ANY FLOW AND STYLE",
    HidePremium = false,
    SaveConfig = true,
...
local function set_style(desired_style)
    while true do
        ...
        if player:FindFirstChild("PlayerStats") and player.PlayerStats:FindFirstChild("Style") then
            if player.PlayerStats.Style.Value ~= desired_style then
                replicated_storage.Packages.Knit.Services.StyleService.RE.Spin:FireServer()
            else
                break
            end
        end
...
local function set_flow(desired_flow)
    while true do
        ...
        if player:FindFirstChild("PlayerStats") and player.PlayerStats:FindFirstChild("Flow") then
            if player.PlayerStats.Flow.Value ~= desired_flow then
                replicated_storage.Packages.Knit.Services.FlowService.RE.Spin:FireServer(desired_flow)
            else
                break
            end
        end
...
notes_tab:AddLabel("also if you got shidou and you want nagi you need to rejoin")
notes_tab:AddLabel("this applies with every style")

local credits_tab = window:MakeTab({ Name = "Credits", Icon = "rbxassetid://4483345998" })
credits_tab:AddSection({ Name = "Credits" })
credits_tab:AddLabel("this ui is based on orion ui")
credits_tab:AddLabel("credits 52wy on discord-- real owner")
